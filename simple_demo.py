...
async def demo_simple_usage():
    ...
    if not audio_file:
        print("⚠️  No test audio files found. Create example.wav or test.mp3 to test.")
        return
...
async def demo_session_management():
    ...
    if not test_files:
        print("⚠️  No test files for session demo")
        return
...
def demo_direct_comparison():
    ...
    print("\n⚖️  Demo 3: Direct vs Event Wrapper Comparison")
    print("=" * 50)
    ...
    if not audio_file:
        print("⚠️  No test files for comparison")
        return
...
async def demo_streaming_simulation():
    ...
    if len(test_files) < 2:
        print("⚠️  Need at least 2 audio files for streaming demo")
...
async def main():
    """Run all demos"""
    print("🌪️  Typhoon ASR Events - Minimal Demo")
    print("=" * 60)
